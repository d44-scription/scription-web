...
it('renders with a given font size', async () => {
  ...
  expect(screen.getByRole('label').style.cssText.includes('font-size: 2rem')).toBe(true)
...
it('renders with a default font size', async () => {
  ...
  // Confirm span has correct font size
  expect(screen.getByRole('label').style.cssText.includes('font-size: 1rem')).toBe(true)
