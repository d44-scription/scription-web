...
it('renders list of notebooks', async () => {
  ...
  expect(screen.getAllByRole('listitem')[0].className.includes('active')).toBe(false);
  expect(screen.getAllByRole('listitem')[1].className.includes('active')).toBe(false);
  ...
  expect(screen.getAllByRole('listitem')[0].className.includes('active')).toBe(true);
  expect(screen.getAllByRole('listitem')[1].className.includes('active')).toBe(false);
  ...
  expect(screen.getByText('This will delete Notebook 1 and all it\'s associated notes. Are you sure you wish to continue?')).toBeInTheDocument();
